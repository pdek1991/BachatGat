...
def index():
    ...
    if selected_group_id:
        # Get the group's profit and bank interest rate directly from the group table
...
def all_groups_report():
    ...
    for group in groups:
        ...
        # Pending Contributions for the group
        pending_contributions = conn.execute(
            "SELECT SUM(amount) FROM transactions WHERE group_id = ? AND type = 'contribution' AND is_cash_payment = 1 AND is_paid = 0",
            (group_id,)
        ).fetchone()[0] or 0.0

        # Total Outstanding Loan Amount for the group
        total_outstanding_loans = 0.0
        active_loans_in_group = conn.execute(
            "SELECT * FROM loans WHERE group_id = ? AND status = 'active'",
            (group_id,)
        ).fetchall()
        for loan in active_loans_in_group:
            accrued_interest = calculate_interest_due(
                loan['outstanding_balance'],
                loan['current_interest_rate'],
                loan['last_interest_calc_date'],
            ...
            total_outstanding_loans += loan['outstanding_balance'] + accrued_interest
        
        # Total Paid Contributions for the group
        total_paid_contributions = conn.execute(
            "SELECT SUM(amount) FROM transactions WHERE group_id = ? AND type = 'contribution' AND is_paid = 1",
            (group_id,)
        ).fetchone()[0] or 0.0

        # Monthly Repayment Status for all loans in the group
        loans_due_this_month = conn.execute(
            "SELECT COUNT(*) FROM loans WHERE group_id = ? AND status = 'active' AND issue_date LIKE ?",
            (group_id, f'{current_month}%') # Simplified: assuming loans issued this month are 'due'
        ).fetchone()[0] or 0
        
        loans_repaid_this_month = conn.execute(
            "SELECT COUNT(DISTINCT loan_id) FROM transactions WHERE group_id = ? AND type = 'loan_repayment' AND date LIKE ?",
            (group_id, f'{current_month}%')
        ).fetchone()[0] or 0

        report_data.append({
            'group_name': group['name'],
            'pending_contributions': round(pending_contributions, 2),
            'total_outstanding_loans': round(total_outstanding_loans, 2),
            'total_paid_contributions': round(total_paid_contributions, 2),
            'loans_due_this_month': loans_due_this_month,
            'loans_repaid_this_month': loans_repaid_this_month,
        })
    conn.close()
    return render_template('all_groups_report.html', 
...
def add_group():
    ...
    conn = get_db_connection()
    try:
        conn.execute('INSERT INTO groups (name, bank_account_details, group_profit, default_interest_rate, bank_interest_rate) VALUES (?, ?, ?, ?, ?)',
                     (name, bank_account_details, 0.0, default_interest_rate, bank_interest_rate))
        conn.commit()
...
def bulk_upload_transactions():
    ...
    if request.method == 'POST':
        ...
        # Use DictReader with tab delimiter if CSV is tab-separated
        ...
        for row_idx, row in enumerate(csv_reader):
            ...
            # Date parsing: Try YYYY-MM-DD first, then DD-MM-YYYY
            date_str_raw = row.get('date', datetime.now().strftime('%Y-%m-%d')).strip()
            parsed_date = None
            for fmt in ['%Y-%m-%d', '%d-%m-%Y']:
                try:
                    parsed_date = datetime.strptime(date_str_raw, fmt).strftime('%Y-%m-%d')
                    break
                except ValueError:
                    continue
            
            if parsed_date is None:
                flash(f"{get_marathi_label('error_processing_row').format(error=f'Invalid date format for {date_str_raw}')}: {get_marathi_label('data_conversion_error').format(error=f'Invalid date format: {date_str_raw}')} - Data: {dict(row)}", 'danger')
                continue
            ...
            try:
                ...
                if not user:
                    # Fix: Ensure 'error' argument is passed to format even if empty
                    flash(f"{get_marathi_label('error_processing_row').format(error='')}: {get_marathi_label('user_not_found_in_group').format(user_name=user_name, group_name=get_group(selected_group_id)['name'])}", 'warning')
                ...
                if transaction_type not in valid_types:
                    # Fix: Ensure 'error' argument is passed to format even if empty
                    flash(f"{get_marathi_label('error_processing_row').format(error='')}: {get_marathi_label('unknown_transaction_type').format(transaction_type=transaction_type)}", 'warning')
                    continue
                ...
                # Fix: Ensure 'error' argument is passed to format
                flash(f"{get_marathi_label('error_processing_row').format(error=get_marathi_label('data_conversion_error').format(error=ve))} - Data: {dict(row)}", 'danger')
                continue
            except Exception as e:
                # Fix: Ensure 'error' argument is passed to format
                flash(f"{get_marathi_label('error_processing_row').format(error=e)} - Data: {dict(row)}", 'danger')
                continue
...
अनिता ज्ञानोबा अडसुळे	contribution	2000	01-12-2021	0	1	
आशा सत्यशील कांबळे	contribution	2000	01-01-2022	0	1	
चैत्राली सचिन कदम	contribution	2000	01-02-2022	0	1	
"""
    # Define the path where the sample CSV will be temporarily saved
    file_path = os.path.join(app.root_path, 'sample_transactions_upload.csv')
...
"""
    # Define the path where the sample CSV will be temporarily saved
    file_path = os.path.join(app.root_path, 'sample_users_upload.csv')
...
अनिता ज्ञानोबा अडसुळे	contribution	1000	01-05-2024	0	1	
आशा सत्यशील कांबळे	loan_disbursement	5000	01-05-2024		24.0	
आशा सत्यशील कांबळे	loan_repayment	1000	20-06-2024	0	1	1
चैत्राली सचिन कदम	interest_paid	50	20-06-2024	0	1	2
अनिता ज्ञानोबा अडसुळे	contribution	500	25-06-2024	1	0	
""")
