...
def calculate_interest_due(original_loan_amount, annual_interest_rate_percent, start_date_str, end_date_str):
    """
    Calculates interest based on the original loan amount for full months elapsed.
    Example: for 100000 loan at 12% annual, monthly interest is 1000.
    If 2 full months passed, interest due is 2000.
    """
    if not original_loan_amount or original_loan_amount <= 0:
        return 0.0
    ...
    while temp_date < end_date:
        ...
        # If the start of the next month is still within or before the end_date,
        # then a full month has passed.
        if temp_date <= end_date:
            months_passed += 1
        else:
            break

    # Calculate monthly fixed interest amount based on original_loan_amount
    monthly_fixed_interest_rate_decimal = (annual_interest_rate_percent / 100.0) / 12.0
    fixed_monthly_interest_amount = original_loan_amount * monthly_fixed_interest_rate_decimal

    total_accrued_interest = fixed_monthly_interest_amount * months_passed
    return round(total_accrued_interest, 2)
        ...
        for user in users:
            ...
            for loan in active_loans:
                accrued_interest = calculate_interest_due(
                    loan['original_amount'], # Pass original_amount
